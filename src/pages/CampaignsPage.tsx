...
export function buildN8nWorkflow(campaign: Campaign, steps: CampaignStep[], templates: EmailTemplate[]) {
  const nodes: object[] = []
  ...
  if (steps.length > 0) {
      ...
      if (step.channel === 'email') {
        ...
        nodes.push({
          ...
          parameters: {
            ...
            from: '',
            to: '={{ $json.phone }}',
            message: tpl?.html_body ?? 'Hi! This is New Empire Remodeling.',
          },
        ...
        nodes.push({
          ...
          typeVersion: 1,
          position: [1400, yOff],
          notes: `${CHANNEL_META[step.channel].label} DM — connect messaging node here.`,
          parameters: {},
        })
      }
    ...
    // No steps yet — just a placeholder
    nodes.push({
      name: 'No Steps Yet',
      type: 'n8n-nodes-base.noOp',
      typeVersion: 1,
      position: [1120, 200],
      notes: 'Add steps in the CRM Campaign Builder and click Sync to populate this workflow.',
      parameters: {},
    })
...
async function syncWorkflowToN8n(campaign: Campaign, steps: CampaignStep[], templates: EmailTemplate[]): Promise<{ id: string; url: string }> {
  const apiKey = getApiKey()
  if (!apiKey) throw new Error('Configure a API Key do n8n na aba Workflows antes de sincronizar.')

  const workflowName = `Campanha: ${campaign.name}`
  const workflow = buildN8nWorkflow(campaign, steps, templates)
  const existingId = campaign.n8n_workflow_id

  let workflowId: string
  if (existingId) {
    const res = await fetch(`${N8N_BASE}/api/v1/workflows/${existingId}`, {
      method: 'PUT',
...
function CreateCampaignModal({ onClose, onCreate }: {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true)
    try {
      const campaign = await onCreate({ name: name.trim(), description: description.trim() || null, color })
      // Auto-create empty n8n workflow
      try {
        await syncWorkflowToN8n(campaign, [], [])
      } catch {
        // silently ignore if n8n key not set yet
      }
      onClose()
    } catch (err) {
    ...
    <div className="flex items-start gap-3 p-4 bg-empire-navy rounded-xl border border-empire-border">
      ...
      <div className="flex-1 grid grid-cols-1 sm:grid-cols-3 gap-3">
        ...
        <div>
          ...
          {compatible.length === 0 && (
            <p className="text-xs text-yellow-500 mt-1">Nenhum template de {meta.label}. Crie um na aba Templates.</p>
          )}
        </div>
        <div>
      ...
      <div className="flex flex-col items-end gap-2">
        ...
        <button onClick={() => onRemove(step._key)} className="text-gray-600 hover:text-red-400 transition-colors p-1">
          <Trash2 size={14} />
        </button>
      </div>
    </div>
...
function CampaignDetail({ campaign: initialCampaign, onBack }: { campaign: Campaign; onBack: () => void }) {
  ...
  useEffect(() => {
    fetchCampaignSteps(campaign.id).then((s) => setSteps(s.map((st) => ({ ...st, _key: st.id }))))
  ...
  const handleSaveAndSync = useCallback(async () => {
    ...
    setError('')
    try {
      const canonical: CampaignStep[] = steps.map((s, i) => ({
        id: s.id ?? '',
      ...
      const { id, url } = await syncWorkflowToN8n(campaign, canonical, templates)
      setCampaign((prev) => ({ ...prev, n8n_workflow_id: id, n8n_workflow_url: url }))
      setSaved(true)
      ...
      <div className="bg-empire-card border border-empire-border rounded-xl overflow-hidden">
        <div className="px-5 py-4 border-b border-empire-border flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-500 text-xs mt-0.5">Salvar atualiza automaticamente o workflow no n8n</p>
          </div>
          <div className="flex items-center gap-2 flex-wrap justify-end">
            {saved && <span className="text-green-400 text-xs flex items-center gap-1"><CheckCircle2 size={13} />Salvo e sincronizado</span>}
            {error && <span className="text-red-400 text-xs max-w-[200px] text-right">{error}</span>}
            ...
            <button onClick={() => void handleSaveAndSync()} disabled={saving}
              className="flex items-center gap-1.5 px-4 py-1.5 rounded-lg bg-empire-gold text-empire-dark text-sm font-semibold hover:bg-empire-gold/90 transition-colors disabled:opacity-50">
          ...
          {steps.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 gap-3 text-center">
              ...
              <p className="text-empire-muted text-xs max-w-xs">
                Adicione etapas de comunicação. Cada etapa usa um canal e template diferentes.
              </p>
              <button onClick={addStep}
      ...
      <div className="space-y-6 max-w-4xl">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-1">
            Gerencie campanhas de nutrição e classifique leads em fluxos multicanal.
          </p>
        </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-empire-border">
                  {campaigns.map((c) => (
                    <tr key={c.id} className="hover:bg-empire-navy/40 transition-colors cursor-pointer" onClick={() => setSelected(c)}>
